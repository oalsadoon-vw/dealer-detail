...
export default function DashboardClient() {
    ...
    fetch("/api/stores")
      .then(async (r) => {
        ...
        try {
          ...
          throw new Error(`Stores fetch failed (${r.status})`);
        }
        if (!r.ok) throw new Error(json?.error ?? `Failed to load stores (${r.status})`);
        return json as Store[];
      })
      .then((s) => {
        setStores(s);
        if (s?.[0]?.id) setStoreId(s[0].id);
      })
  ...
  useEffect(() => {
    ...
    setError(null);
    setData(null);
    fetch(`/api/dashboard?storeId=${storeId}&startDate=${startDate}&endDate=${endDate}`)
      .then(async (r) => {
        const text = await r.text();
        let json: any = null;
        try {
          ...
          throw new Error(`Dashboard failed (${r.status})`);
        }
